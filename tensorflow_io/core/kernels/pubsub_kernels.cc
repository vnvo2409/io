  ...
  Status Read(std::function<Status(const TensorShape& shape, Tensor** id_tensor,
    ...
    auto session = subscriber.Subscribe(
        [&](pubsub::Message const& m, pubsub::AckHandler h) {
          message_ids.emplace_back(m.message_id());
          message_datas.emplace_back(m.data());
    ...
    std::copy(message_ids.begin(), message_ids.end(),
              id_tensor->flat<tstring>().data());
    std::copy(message_datas.begin(), message_datas.end(),
              data_tensor->flat<tstring>().data());
    std::copy(message_times.begin(), message_times.end(),
